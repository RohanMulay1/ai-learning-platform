...
export function PomodoroWidget() {
  ...
  const progress = phase === "break" ? 1 : secondsLeft / total;
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {/* Floating popover */}
      {open && (
        <div className="absolute top-full right-0 mt-2 z-50 w-52 bg-white border border-gray-200 shadow-2xl rounded-2xl p-5">
          {/* Header */}
          <div className="flex items-center justify-between mb-4">
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Pomodoro</span>
            <button
              onClick={() => setOpen(false)}
              className="w-5 h-5 flex items-center justify-center rounded-md text-gray-300 hover:text-gray-500 hover:bg-gray-100 transition-colors"
            >
          ...
          <div className="flex justify-center mb-4">
            <div className="relative w-24 h-24">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span className="text-[9px] uppercase tracking-widest font-bold mt-0.5" style={{ color: phaseColor }}>{phaseLabel}</span>
              </div>
            </div>
          </div>
